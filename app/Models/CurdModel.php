...
use CodeIgniter\Model;
    ...
    class CurdModel extends Model
    {
		...
		public function update_mlm_session()
		{
			$session = session()->get('mlmlogin');
            $q = $this->db->table('mlm_login_history')->where(array('id'=>$session['login_id']))->set(array('last_activity_time'=>date('Y-m-d H:i:s')))->update();
            if($q){ return true;}
        }
		...
		{
            $builder = $this->db->table($table)->where($where)->delete();
